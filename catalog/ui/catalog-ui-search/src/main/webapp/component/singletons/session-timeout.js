...
var idleTimeoutThreshold = 900000; // Length of inactivity that will trigger user timeout (15 minutes in ms) 
// See STIG V-69243
...
var sessionTimeoutModel = new(Backbone.Model.extend({
    ...
    handleShowPrompt: function() {
        if (this.get('showPrompt')) {
            this.stopListeningForUserActivity();  //comment out to reset idle during prompt showing (so they don't have to explicitly click to reset timer)
        } else {
    ...
    resetIdleTimeoutDate: function () {
      ...
      //  idleTimeoutThreshold = 900000;  // comment out to test tiny intervals (set small for first, then on first update set high)
    },
